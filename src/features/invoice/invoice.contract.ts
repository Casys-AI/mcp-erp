...
export const INVOICE_TOOLS: readonly ErpToolDefinition[] = [
  ...
  {
    name: "erp.sales_invoice_create",
    description:
      "Create a sales invoice (draft) in normalized form. mode 'preview' validates without writing; 'commit' writes.",
    inputSchema: {
      type: "object",
      properties: {
        erpType: ERP_TYPE_SCHEMA,
        ...
        date: {
          type: "string",
          description:
            "Posting date (YYYY-MM-DD). Defaults to today when omitted.",
        },
        dueDate: {
          type: "string",
          description:
            "Payment due date (YYYY-MM-DD). Must not be before date.",
        },
      },
  ...
  {
    name: "erp.sales_invoice_submit",
    description:
      "Submit a sales invoice (draft → submitted). Transitions the document from draft (docstatus 0 / statut 0) to submitted (ERPNext docstatus 1) or validated (Dolibarr statut 1). mode 'preview' resolves the native plan without any HTTP call; mode 'commit' performs the transition and the result carries a normalized lifecycleState.",
    inputSchema: {
      type: "object",
      properties: {
        erpType: ERP_TYPE_SCHEMA,
