...
`erp-platform` route MCP `tools/call` to a customer's LAN without exposing the
ERP publicly.

This is the **only** piece of the Casys stack that runs on customer hardware.
Keep its blast radius small: read-only file access, no inbound port, no
...
authenticates, and forwards every incoming `tool.call` to the local ERP through
the matching adapter.

## Install

While `@casys/mcp-bridge` and `@casys/mcp-erp` are not yet JSR-published, clone
the three sibling repos under a common parent:
...
Future: `deno install -A -g jsr:@casys/mcp-erp-local-agent/cli` once published.
...
`via: "header"` is the only accepted bearer placement. `via: "query"` is
**rejected at config parse time** (`CONFIG_FIELD_INVALID: endpointAuth.via …`)
because most WebSocket proxies log the request URL — putting the token in the
query string leaks it to access logs, systemd journals, and crash reports.

If your environment cannot pass headers, use the `headers` variant with a
non-standard header name carried separately by the proxy.
...
`apiUrl` must use `http:` or `https:`. `sandbox: true` should be the norm during
smoke-tests.
...
Required Deno permissions: `--allow-read --allow-net` (already wired in the
`agent` task).
...
`SIGINT` / `SIGTERM` triggers a graceful shutdown: in-flight tool calls finish,
the WS closes, the ERP adapter disposes, then exit 0.

## Error codes

All errors emitted at startup are machine-readable. Match the `CODE: message`
shape per AX principle #4 (`code` / `context` / `recovery`).

| Code                        | Meaning                                              |
| --------------------------- | ---------------------------------------------------- |
| `CLI_CONFIG_REQUIRED`       | No `--config <path>` flag.                           |
| `CONFIG_FILE_NOT_FOUND`     | The config file at the given path does not exist.    |
| `CONFIG_FILE_UNREADABLE`    | The config file exists but the user cannot read it.  |
| `CONFIG_JSON_INVALID`       | The config file contains malformed JSON.             |
| `CONFIG_UNEXPECTED_ERROR`   | Unclassified failure while loading the config.       |
...
CLI exit codes: `2` for any `CONFIG_*` error, `4` for a terminal
`NetworkRelayError` (auth-style WS close codes), `1` for unknown.
...
- The `tool.call` payloads — every call is dispatched through the ERP adapter,
  which enforces the ERP's own auth model on each request.
- Other agents on the same network — there is no inter-agent communication.

Credentials live in memory only. The config file is read once at start; nothing
is written back. Logs do not include credential fields. Verify with
`deno task check` (the test suite asserts no credential leak in `console.log`
calls).

## Troubleshooting

See
[`erp-platform/docs/runbooks/local-erp-agent-smoke-test.md`](../../../erp-platform/docs/runbooks/local-erp-agent-smoke-test.md)
...
local-agent context (transport, adapter registry, ERP catalogs) lives upstream
in `@casys/mcp-bridge` and `@casys/mcp-erp` — do not duplicate.
